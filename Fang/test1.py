...
def week(): # enter the how many day are considered as weekday,return as tuple
    ...
    def inner(day):
        ...
        # on weekday
        if day in list(price_dict.keys())[:5]:
            if 0 <= total_minutes <= 15:#first 15 min free
                amount = 0
                return amount
            elif 15 < total_minutes <= 185: # 3:05 = 185 minutes
                amount = fir_charge
                return amount
            # amount charge based on hour unit
            amount = fir_charge + (hour - fir_hours) * sub_charge
            if minute // sub_min != 0:

            # test data : 4:06 > 246, 5:06 > 306, 6:06 > 366 
            # amount charge based on minute unit
        return amount
    amount = inner(day)
